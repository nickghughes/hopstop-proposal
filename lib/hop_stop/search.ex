...
defmodule HopStop.SearchByLocation do
  def get_breweries(query_str) do
    headers = [
      "x-rapidapi-key": System.get_env("BREWERY_API_KEY"),
		  "x-rapidapi-host": "brianiswu-open-brewery-db-v1.p.rapidapi.com"
    ]
    resp = HTTPoison.get!("https://brianiswu-open-brewery-db-v1.p.rapidapi.com/breweries" <> query_str, headers)
  ...
  def get_breweries_by_location(location, query_str \\ nil, page \\ 1, results \\ []) do
    ...
    cond do
      length(results) < page_size and String.length(query) > 0 -> 
        get_breweries_by_location(location, broadened_query, 0, results)
      true -> %{"page" => page, "query" => query, "results" => results}
    end
  end

  def get_next_page(prev_result_hash) do
    get_breweries_by_location(nil, prev_result_hash["query"], prev_result_hash["page"]+1)
  end
  ...
  def lat_lng_to_breweries(latitude, longitude) do
